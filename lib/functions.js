...
const options = (start) => { 
	const startOptions = {
		...
		//disableSpins: true,
		headless: true,
		komiProcessOnBrowserClose: true,
		qrTimeout: 0,
...
const vote = async (komi, message, pollfile, voterslistfile) => {
	const isVotedP = await isvoted(message, voterslistfile)
	if (isVotedP) { await komi.reply(message.chatId, mylang(config.Language).polliv(), message.id, true);return }
	let data = await readJsonFile(pollfile)
	if (data['candis'] === 'null') { await komi.reply(message.chatId, mylang(config.Language).nocand(), message.id, true);return }
	let arr = data['candis']
	for (let i = 0; i < arr.length; i++) { if (message.body.includes((i + 1).toString())) { await addvote(komi, message, i, pollfile, voterslistfile);return } }
	await komi.reply(message.chatId, mylang(config.Language).fail(), message.id, true)
}

// Adiciona candidatos na lista
const add = async (komi, message, candi, pollfile, voterslistfile) => {
	let data = await readJsonFile(pollfile)
	if (data['candis'] === 'null') {
	...
	await saveJsonFile(pollfile, data);await komi.reply(message.chatId, mylang(config.Language).addcand(candi), message.id, true)
}

// Salva os votos no arquivo JSON
const addvote = async (komi, message, num, pollfile, voterslistfile) => {
	...
	op = op + ls;op = op + mylang(config.Language).howvote();await komi.reply(message.chatId, op, message.id, true)
...
// Adquire a votação
const get = async (komi, message, pollfile, voterslistfile) => {
	...
	await komi.reply(message.chatId, op, message.id, true)
}

// Cria uma votação
const resetp = async (komi, message, polltitle, pollfile, voterslistfile) => {
	let base = { title: polltitle, candis: 'null' }
	await saveJsonFile(pollfile, base);await komi.reply(message.chatId, mylang(config.Language).startvote(polltitle), message.id)
